...
let GOOGLE_API_KEY = localStorage.getItem("user_gemini_key") || "";
const apiKeyInput = document.getElementById("api-key-input");
const saveKeyBtn = document.getElementById("saveKeyBtn");

if (GOOGLE_API_KEY) apiKeyInput.value = GOOGLE_API_KEY;

saveKeyBtn.addEventListener("click", () => {
    // ... your existing save key code ...
});

// --- NEW: Model Selection Logic ---
const modelSelect = document.getElementById("model-select");
// Load saved model or default to 3.1 Flash Lite
let CURRENT_MODEL = localStorage.getItem("user_gemini_model") || "gemini-3.1-flash-lite";
modelSelect.value = CURRENT_MODEL;

// Update and save the model when the user changes the dropdown
modelSelect.addEventListener("change", (e) => {
    CURRENT_MODEL = e.target.value;
    localStorage.setItem("user_gemini_model", CURRENT_MODEL);
});


// Populate the input field if a key exists
if (GOOGLE_API_KEY) {
    apiKeyInput.value = GOOGLE_API_KEY;
}
...
saveKeyBtn.addEventListener("click", () => {
    ...
    if (inputVal) {
        GOOGLE_API_KEY = inputVal;
        localStorage.setItem("user_gemini_key", GOOGLE_API_KEY);
...
const requestApiResponse = async (incomingMessageElement) => {
    ...
    if (!GOOGLE_API_KEY) {
        isGeneratingResponse = false;
        messageTextElement.innerText = "Please enter and save your Gemini API Key at the top right first!";
        messageTextElement.closest(".message").classList.add("message--error");
        incomingMessageElement.classList.remove("message--loading");
        return;
    }

  // Define the URL here so it always uses the current GOOGLE_API_KEY and CURRENT_MODEL
    const API_REQUEST_URL = `https://generativelanguage.googleapis.com/v1beta/models/${CURRENT_MODEL}:generateContent?key=${GOOGLE_API_KEY}`;
...
newChatBtn.addEventListener("click", () => {
    // Optional: Add a confirm dialog if you want to warn them first
    if (confirm("Start a new chat? This will clear your current conversation.")) {
        localStorage.removeItem("saved-api-chats");

        // Reload the UI to reflect the empty state
        loadSavedChatHistory();
